...
class Fighter {
  ...
  _getCurrentSprite() {
    // Use jump_attack as a highly dynamic forward-leaning pose for the dash
    if (this.isUltimateDash && this.sprites.jump_attack) return this.sprites.jump_attack;
  ...
  draw(ctx, canvasW, canvasH) {
    ...
    if (this.state === 'ultimate_dash') {
      ...
      if (auraImg) {
        ...
        ctx.globalAlpha = 0.55; // Tiết chế chút, transparent
  ...
  _drawBlockShield(ctx) {
    ...
    ctx.shadowBlur = 20;
    ctx.beginPath();
    ctx.arc(this.centerX + dir * 30 + sh.x, this.centerY + sh.y, 50, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
